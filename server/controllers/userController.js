...
export const sendConnectionRequest = async(req, res) => {
    try {
        ...
        // Check if the user has sent more than 20 connection requests in the last 24 hours or not
...
export const getUserConnections = async(req, res) => {
    try {
        const { userId } = req.auth();
        const user = await User.findById(userId).populate('followers following connections');
        ...
        res.json({success: true, followers, following, connections, pendingConnections});
    } catch (error) {
        console.log(error);
        res.json({success: false, message: error.message})
    }
...
export const acceptConnectionRequest = async(req, res) => {
    try {
        ...
        const connection = await Connection.findOne({from_user_id: userId, to_user_id: id});
